...
export default class FailsPreview extends React.Component {
  render() {
    ...
    const entryFails = entry.getIn(["data", "fails"]);
    const fails = entryFails ? entryFails.toJS() : [];

    return <div>
      <Jumbotron image={image} title={entry.getIn(["data", "title"])} />
      <div className="bg-off-white pv4">
        <div className="mw7 center ph3 pt4">
          {fails.map(({text, heading, imageUrl}, i) =>
